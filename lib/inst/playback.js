...
class PlaybackClass {
    ...
    play() {
        this.render().then(() => {
            ...
            this.timer.setInterval(this.increment.bind(this), '', this.timing + 'u');
        });
    }

    stop() {
        this.timer.clearInterval();
        this.playing.forEach(([Inst, midi, timer]) => {
            timer.clearTimeout();
            Inst.stopMidi(midi);
        });
        this.playing.clear();
        ...
        this.notes.forEach((inst) => {
            ...
            if (notes) {
                notes.forEach((note) => {
                    ...
                    let playing = [Inst, note.midi, timer];
                    this.playing.add(playing);

                    timer.setTimeout(() => {
                        Inst.stopMidi(note.midi);
